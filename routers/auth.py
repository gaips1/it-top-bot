...
async def login(message: Message, state: FSMContext):
    ...
    usr = tap(message.from_user.id, username, password)
...
async def relogin_handler(message: Message, state: FSMContext):
    ...
    if not user:
        return await auth_handler(message.from_user.id, state)
    
    return await auth_handler(message.from_user.id, state, first_time=False)